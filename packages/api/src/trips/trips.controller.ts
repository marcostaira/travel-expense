...
export class TripsController {
  ...
  create(@CurrentUser() user: CurrentUserData, @Body() createTripDto: CreateTripDto) {
  ...
  findAll(
    ...
    return this.tripsService.findAll(user.tenantId, user.userId, user.role, page, limit, filters);
  }
  ...
  update(
    ...
    return this.tripsService.update(user.tenantId, id, user.userId, user.role, updateTripDto);
  }
  ...
  @ApiResponse({ status: 200, description: 'Viagem enviada para aprovação com sucesso' })
  ...
  approve(@CurrentUser() user: CurrentUserData, @Param('id') id: string, @Body('notes') notes?: string) {
    return this.tripsService.approve(user.tenantId, id, user.userId, notes);
  }
  ...
  reject(@CurrentUser() user: CurrentUserData, @Param('id') id: string, @Body('reason') reason: string) {
    return this.tripsService.reject(user.tenantId, id, user.userId, reason);
  }
  ...
  @Post(':id/complete')
  @ApiOperation({ summary: 'Concluir viagem' })
  @ApiResponse({ status: 200, description: 'Viagem concluída com sucesso' })
  complete(@CurrentUser() user: CurrentUserData, @Param('id') id: string) {
    return this.tripsService.complete(user.tenantId, id, user.userId, user.role);
  }
}